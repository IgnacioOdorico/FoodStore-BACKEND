...
#      - Ejemplos: role:admin, role:pedidos, role:cocina, role:user
...
#
#   SUSCRIPCIÓN A PEDIDO (clientes y staff):
#     1. Cliente envía: {"action": "subscribe-order", "order_id": 5}
#     2. Backend valida que el pedido pertenece al usuario (solo para USER)
...
#   → broadcast_to_roles(["pedidos", "cocina", "admin"], "PEDIDO_CONFIRMADO", pedido_data)
#
# ─── SEGURIDAD ─────────────────────────────────────────────────────────────────
#
...
#
#   Backend → Cliente:
#     {"event": "PEDIDO_NUEVO",         "data": {...pedido...}}
...
class ConnectionManager:
    ...
    def __init__(self) -> None:
        ...
        #      en múltiples rooms (ej: admin en role:admin + room de pedido)
    ...
    async def connect(self, websocket: WebSocket, role: str, user_id: int) -> None:
        ...
        Args:
            ...
            role:      El rol del usuario (ej: "ADMIN", "PEDIDOS", "COCINA", "USER")
    ...
    def join_order_room(self, websocket: WebSocket, order_id: int) -> None:
        ...
        Los clientes (role:user) usan esto para recibir actualizaciones de su
        pedido. El router valida la propiedad del pedido antes de llamar este método.
    ...
    async def broadcast_to_roles(
        ...
        Si un socket está en varias rooms (ej: un usuario con múltiples roles),
        solo recibe el evento una vez. Esto evita duplicados en el frontend.

        Ejemplo:
          broadcast_to_roles(["pedidos", "cocina", "admin"], "PEDIDO_CONFIRMADO", data)
          → cajeros, cocineros y admins reciben el evento, sin duplicados

        Args:
            roles:      Lista de nombres de rol (ej: ["pedidos", "cocina", "admin"])
            event_type: Tipo de evento
            data:       Datos del pedido
        """
    ...
    async def broadcast(self, event_type: str, data: dict[str, Any]) -> None:
        """
        Broadcast a TODAS las conexiones activas (método de compatibilidad).

        Este método existe para no romper llamadas existentes durante la migración.
        En el flujo normal se prefieren broadcast_to_order() y broadcast_to_roles()
        por ser más selectivos y eficientes.
    ...
    async def _emit_to_room(self, room: str, event_type: str, data: dict[str, Any]) -> None:
        ...
        """
        if room not in self.rooms:
            logger.info(f"Evento {event_type} descartado (room {room} vacía o inexistente).")
            return
...
#
# Se importa desde:
#   - router.py (pedidos): para connect() y disconnect() en el handshake/desconexión
#   - service.py (pedidos): para broadcast_to_order() y broadcast_to_roles() al cambiar estado
#
# Uso:
#   from app.core.websocket import manager
#   await manager.broadcast_to_roles(["pedidos", "cocina"], "PEDIDO_CONFIRMADO", data)
#
